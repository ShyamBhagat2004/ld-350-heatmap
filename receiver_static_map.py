...
# List to store coordinates of each lightning strike
strike_coordinates = []
...
def process_message(message):
    if message.startswith("$WIMLI,"):
        ...
        if data:
            ...
            strike_coordinates.append((new_lat, new_lon))
            print(f"Lightning strike data -> New Coordinates: Latitude {new_lat}, Longitude {new_lon}")
    elif message.startswith("$WIMLN*"):
        pass  # Ignoring noise messages
    else:
        pass  # Ignoring unrecognized message types
...
def create_heatmap():
    ...
    map_center = [base_lat, base_lon]
    map = folium.Map(location=map_center, zoom_start=10)
    # Add a HeatMap
    HeatMap(strike_coordinates).add_to(map)
